...
type Props = {
  world: WorldState;
  onClose: () => void;
  onWorldUpdate: (w: WorldState) => void; // 保存成功后局部更新伏笔账（右栏/脉络联动）
...
export const ForeshadowModal: React.FC<Props> = (p) => {
  ...
  async function call(action: string, payload: Record<string, unknown>): Promise<Foreshadow[] | null> {
    ...
    try {
      ...
      const data = (await res.json()) as { ok?: boolean; foreshadowing?: Foreshadow[]; error?: string };
      if (!data.ok) throw new Error(data.error ?? "操作失败");
      const next = data.foreshadowing ?? [];
      setList(next);
      // 同步到全局 world（右栏伏笔账/脉络联动）
      p.onWorldUpdate({ ...p.world, foreshadowing: next });
      return next;
    } catch (e) {
  ...
  async function addForeshadow() {
    ...
    if (ok) { setNewText(""); setNewNote(""); setNewPlantedAt(""); p.showToast("已新增伏笔"); }
  }
      ...
      <div className="fs-modal" onClick={(e) => e.stopPropagation()}>
        <div className="mem-modal-head">
          ...
          <div className="mem-tabs">
            {(["all", ...STATUS_ORDER] as const).map((k) => (
              <button key={k} className={filter === k ? "mem-tab on" : "mem-tab"} onClick={() => setFilter(k)}>
        ...
        <div className="mem-modal-body">
          ...
          <div className="fs-add">
            <input className="fs-add-text" placeholder="伏笔内容（必填）…" value={newText} onChange={(e) => setNewText(e.target.value)} disabled={busy || p.taskActive} />
            <input className="fs-add-note" placeholder="备注（可选）" value={newNote} onChange={(e) => setNewNote(e.target.value)} disabled={busy || p.taskActive} />
            <input className="fs-add-ch" placeholder="埋设章（默认最新章）" value={newPlantedAt} onChange={(e) => setNewPlantedAt(e.target.value)} disabled={busy || p.taskActive} inputMode="numeric" />
            ...
            <tbody>
              {shown.map((f) => (
                <tr key={f.id}>
                  ...
                  <td>
                    <select className="fs-cell-select" value={f.status} onChange={(e) => void updateForeshadow(f, { status: e.target.value as Foreshadow["status"] })} disabled={busy || p.taskActive}
                      title="切换状态：已回收 = 伏笔兑现（可删除）">
                  ...
                  <td>
                    <button className="btn-save btn-danger-sm" style={{ fontSize: "0.68rem" }} onClick={() => void deleteForeshadow(f)} disabled={busy || p.taskActive}
                      title={f.status !== "resolved" && f.plantedAt < p.world.nextChapter ? "已埋入正文的伏笔不可删除（需先回收为已解决）" : "删除该伏笔"}>
                      <Trash2 size={12} /> 删除
                    </button>
                  </td>
                </tr>
          ...
          <div className="task-note" style={{ marginTop: "0.5rem" }}>
            ⚠ 已埋入正文的伏笔不可删除（用户决策：伏笔不允许放弃）——需先切换为「已回收」再删除；修改会记入操作日志并触发账本对齐。
          </div>
        </div>
      </div>
    </div>
