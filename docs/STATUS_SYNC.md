...
- **修复：推进按钮永久 loading（阶段 5 回归）**：`advance()` 结束后原依赖常驻轮询清 `advancePhase`；阶段 5 关闭轮询后该值残留 → `taskActive` 永久 true → 按钮永久 loading 直到刷新。修复：① `advance()`/`finishRestoreAdvance` finally 显式清 `advancePhase`；② 服务端 step 完成（done/pending-commit/failed）广播 `task-status(kind:"advance")`，前端 `onTaskStatus` 统一清——覆盖底部按钮/聊天/多 tab 所有发起路径，不依赖本页 SSE 或轮询。
