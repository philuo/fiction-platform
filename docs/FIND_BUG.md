...
### 2026-08-15 第三批验收收尾总结

- **环境**：隔离实例 `C:\workspace\moshift-qa0814`（独立 data/app.db，端口 3300），账号 `qa0814`，测试书《镜像罪案》（6 章）与《云中谋杀》（立项测试书）；浏览器为 headless Chrome v151 + playwright-core 真实 UI 操作；全部 QA 脚本在 `.reasonix/qa/`（gitignored），证据图在 `shots/qa0814/`。
- **批次 A（105 条中枢问法）**：覆盖全部 55 类意图的真实中文问法——93 条首跑通过，12 条偶发超时（高频连发排队，逐条重试 12/12 全过）；发现并修复 BUG-038（open_* 顶掉中枢抽屉，`b0edee2`）。写类问法未点确认时世界状态零变化（确认流正确）。
- **批次 B（多 tab / 刷新）**：双 tab 并发聊天实时同步 PASS（探针消息双端一致）；流式中刷新恢复 PASS（无幽灵 loading、终态收敛）；异步任务触发后刷新 PASS（预览卡与后端 job 状态一致）。
- **批次 C（异步任务重试）**：advance 推进×2（写+审+记账全流程，含草稿合并入册）、章节插画×2（media-plan→image 全链路 succeeded）、角色视觉×2（新建角色触发头像+立绘 succeeded）、自动连载启停×1（CMD-N03/N13 succeeded）、抽卡×2、整书评估×2、一致性巡检×2、new-story 立项×2——除视频外每类任务至少 2 遍，全部成功终态；发现并修复 BUG-039（SSE 卡缺 cardId 导致当回合执行被吞，`ecf1866`）。关闭浏览器后重进：后端状态恢复可见 PASS。
- **批次 D（重启与并发）**：advance 写章 running 期间：聊天发送被串行锁禁止（预期设计）、左栏 tab 切换/伏笔账弹窗等非聊天 UI 正常；任务 running 中杀服务进程 → 重启成功 → advance job 正确标记 `interrupted`（"服务重启中断了任务；已核对持久状态，无法证明任务完成"），无残留 pending 草稿、世界数据一致（6 章完整）；重启后刷新页面会话历史完整恢复、无幽灵 loading。
- **缺陷统计**：本批确认 2 个真实缺陷（BROWSER-BUG-038 P1、BROWSER-BUG-039 P1），均已修复、真实浏览器复验通过并推送（`b0edee2`、`ecf1866`、`9fc61b9`）。全程应用 console warning/error 为 0（排除浏览器插件噪声）。
- **未覆盖**：视频生成（按用户要求除外）；LLM 上游 120s 超时/503 限流为免费渠道环境性问题（服务端有重试兜底，非本系统缺陷）。
