...
/** 测试壳：把 hook 暴露出来断言 */
function Harness(props: { title: string | null; log: (e: string) => void; onReconnected?: () => void }) {
...
function mountHarness(title: string | null, log: (e: string) => void, onReconnected?: () => void): { root: Root; el: HTMLElement } {
  const mount = document.createElement("div");
  document.body.appendChild(mount);
  const root = createRoot(mount);
  root.render(React.createElement(Harness, { title, log, onReconnected }));
...
test("auto-status / task-status 分发到对应回调", async () => {
  const log: string[] = [];
  const { root } = mountHarness("书C", (e) => log.push(e));
  await afterMount();
  const ws = FakeWebSocket.instances[0];
  ws.open();
  await tick(20);
  ...
  await tick(20);
  expect(log).toEqual(["auto-status", "task-status"]);
  root.unmount();
});
