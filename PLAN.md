# AI 小说 — 功能模块规划（bun + React 19，历史规划）

> 核心理念：**把小说生成做成一个"游戏会话"**，而不是一次性 prompt。
>
> 本文保留产品路线和技术决策历史；当前模块边界、状态源和开发流程见 [docs/INSTRUCTION.md](docs/INSTRUCTION.md)。
...
- [x] 章节流式展示（阶段事件 SSE + 未入册草稿增量展示；完成后以权威章节替换，失败/中断明确收敛）
...
- **Phase 3**：增强（章节流式正文已完成；流派模板、角色扮演、Web 可视化仍为后续增强）
