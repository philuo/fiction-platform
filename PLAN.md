...
| 服务 | 端点 | 认证 | 说明 |
|---|---|---|---|
| Agnes AI | `https://api.agnes-ai.cn/v1` | `Authorization: Bearer <key>` | OpenAI 兼容 chat/completions，`agnes-2.5-flash`（512K ctx、streaming、tool calling、免费额度，回退 `agnes-2.0-flash`） |
...
- [x] 免费额度友好：失败重试 + 退避；模型回退（`agnes-2.5-flash` → `agnes-2.0-flash`）
